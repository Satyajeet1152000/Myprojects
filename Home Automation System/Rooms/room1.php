				...
				if($qry)
				{
					while ($getdata = mysqli_fetch_assoc($qry)) 
					{
						...
						<tr>
							...
							<td> 	 
								<input type="checkbox" id="<?php echo $getdata['device_name'].'-room1'; ?>" 
									class = "toggle_btn"
									onclick="btnupdate('<?php echo $getdata['device_name'].'-room1'; ?>','<?php echo $getdata['device_name']; ?>','room1')" <?php echo $getdata['status'] == '1' ? 'checked' : '' ;?>>
							</td>
						</tr>
						<?php
					}
